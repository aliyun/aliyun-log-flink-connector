...
### 新增 Source / SQL / Producer 参数说明

本节只说明新增接口：FLIP-27 `AliyunLogSource`、SQL Connector、FLIP-27 `AliyunLogSink`。旧 `FlinkLogConsumer` / `FlinkLogProducer` 属于老 DataStream 接口，后续会删除，参数不要混用到本节接口中。除特别说明外，时间单位均为毫秒，Unix 时间戳单位为秒。

#### Source 参数说明

`AliyunLogSource` 通过 `env.fromSource(...)` 使用，读取状态由 Flink checkpoint 保存；如果配置了 ConsumerGroup，还可以按配置将 checkpoint 提交到日志服务服务端。
...
| `setFallbackPosition(StartingPosition)` | 否 | `earliest` | 当起始位置为 checkpoint 且服务端没有 checkpoint 时使用的兜底位置，不能设置为 checkpoint。 |
...
SQL Sink 内置写入元数据列如下，声明后不会作为普通 content 写入：
...
新增写入端使用 FLIP-27 Sink API：`AliyunLogSink`。它基于日志服务 Producer SDK 异步发送数据，并在 Flink checkpoint 或作业结束时等待已提交请求完成，提供 at-least-once 语义；日志服务 Producer 不提供可与 Flink 协调的事务协议，因此不声明 exactly-once。

自定义序列化使用 `AliyunLogSerializationSchema<T>`，一个输入元素可以通过 `Collector<SinkRecord>` 输出零条、一条或多条 SLS 记录。新 Sink 不再兼容旧 `LogSerializationSchema` / `LogSerializationSchemaV2`。
...
| `setProperty(String key, String value)` / `setProperties(Properties properties)` | 否 | 无 | 设置 Producer 高级参数，见下方参数。 |
...
### 1. FlinkLogConsumer（旧 DataStream 接口）
...
### 2. FlinkLogProducer（旧 DataStream 接口）
