...
#### 1.6 FLIP-27 Source 参数说明

本节只说明新增的 `AliyunLogSource`，不包含后续会删除的老 `FlinkLogConsumer` 接口。除特别说明外，时间单位均为毫秒，Unix 时间戳单位为秒。

##### 1.6.1 Builder 参数

| Builder 方法 | 是否必填 | 默认值 | 含义 |
|------|------|------|------|
...
| `setConsumerGroup(String consumerGroup)` | 否 | 无 | 日志服务 ConsumerGroup 名称，用于读取或提交服务端 checkpoint。使用 checkpoint 起始位置或需要提交消费进度时应设置。 |
| `setStartingPosition(StartingPosition startingPosition)` / `setStartingPosition(String startingPosition)` | 否 | `earliest` | 消费起始位置。支持 `earliest`、`latest`、`checkpoint` 或 Unix 秒级时间戳；兼容 `begin_cursor`、`end_cursor`、`consumer_from_checkpoint`。 |
| `setFallbackPosition(StartingPosition fallbackPosition)` | 否 | `earliest` | 当起始位置为 checkpoint 且服务端没有 checkpoint 时使用的兜底位置。不能设置为 checkpoint。 |
| `setSplitAssigner(AliyunLogSplitAssigner splitAssigner)` | 否 | `ModuloSplitAssigner` | shard 到 source reader 的分配策略。默认按 shard id 和并行度取模，也可以使用 `RoundRobinSplitAssigner` 或自定义实现。 |
| `setProperty(String key, String value)` / `setProperties(Properties properties)` | 否 | 无 | 设置高级参数，见下表。 |

##### 1.6.2 高级参数

| 参数 key | 是否必填 | 默认值 | 含义 |
|------|------|------|------|
| `ConfigConstants.STOP_TIME` (`stop.time`) | 否 | 无 | 停止消费的 Unix 秒级时间戳。读取到该时间点后停止对应 shard 的消费，适合离线补数据场景。 |
...
#### 1.7 SQL Connector 参数说明

SQL Connector 使用 `WITH` 参数，connector 标识为 `aliyun-log`。本节只说明 SQL Connector 的参数，不和 Java Source Builder 参数混用。
...
| `endpoint` | 是 | 无 | 日志服务 Endpoint，例如 `cn-hangzhou.log.aliyuncs.com`。 |
| `project` | 是 | 无 | 要消费的日志服务 Project。 |
| `logstore` | 是 | 无 | 要消费的 Logstore。 |
| `access-key-id` | 是 | 无 | 访问日志服务的 AccessKey ID。 |
| `access-key` | 是 | 无 | 访问日志服务的 AccessKey Secret。 |
| `consumer-group` | 否 | 无 | 日志服务 ConsumerGroup 名称，用于读取或提交服务端 checkpoint。使用 checkpoint 起始位置或需要提交消费进度时应设置。 |
| `scan.startup.mode` | 否 | `earliest` | 消费起始位置。支持 `earliest`、`latest`、`checkpoint` 或 Unix 秒级时间戳。 |
| `scan.startup.default-position` | 否 | `earliest` | 当起始位置为 checkpoint 且服务端没有 checkpoint 时使用的兜底位置。不能设置为 checkpoint。 |
| `checkpoint.mode` | 否 | `on-checkpoints` | 服务端 checkpoint 提交模式。支持 `on-checkpoints`、`periodic`、`disabled`。 |
| `commit.interval.ms` | 否 | `10000` | `periodic` 模式下提交服务端 checkpoint 的间隔。 |
| `fetch.interval.ms` | 否 | `100` | 当一次拉取没有返回数据时，下次拉取前的等待间隔。 |
| `max.number.per.fetch` | 否 | `100` | 单次从单个 shard 拉取的最大 LogGroup 数量。 |
| `shards.discovery.interval.ms` | 否 | `60000` | 发现 shard split/merge 的周期。 |
| `stop.time` | 否 | 无 | 停止消费的 Unix 秒级时间戳。 |
| `ignore-parse-errors` | 否 | `false` | SQL 字段类型转换失败时的处理方式。`false` 表示抛出异常；`true` 表示该字段输出为 `NULL`。 |
| `region.id` | 使用 `v4` 时必填 | 无 | V4 签名使用的地域 ID。 |
| `signature.version` | 否 | `v1` | 请求签名版本，支持 `v1`、`v4`。 |
...
#### 1.8 关联 API 与 RAM 权限设置
Source 读取会用到的阿里云日志服务接口如下：
* GetCursorOrData
...
子用户使用 Source 读取日志服务数据时，需要授权如下 RAM Policy：
...
* 初始化配置参数 `Properties`。Producer 是写入端，参数独立于 Source 和 SQL Connector。一般情况下使用默认值即可，特殊场景可以按下表调整：
