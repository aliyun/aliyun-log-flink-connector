...
public class AliyunLogSinkIntegrationTest {
    ...
    private static final int FAILOVER_AFTER_RECORDS = 3_000;

    @Test
    public void testSinkWritesRecordsReadableBySource() throws Exception {
        Assume.assumeTrue(Boolean.getBoolean("aliyun.sls.integration"));

        runIntegrationTest(false, null);
    }

    @Test
    public void testSinkDoesNotLoseRecordsAfterFailover() throws Exception {
        Assume.assumeTrue(Boolean.getBoolean("aliyun.sls.integration"));

        File failureMarker = new File(System.getProperty("java.io.tmpdir"),
                "aliyun-log-sink-failover-" + System.nanoTime());
        try {
            ReadResult result = runIntegrationTest(true, failureMarker.getAbsolutePath());
            assertEquals("Sink failover test did not trigger an intentional failure",
                    true, failureMarker.exists());
            System.out.println("Sink failover duplicate ids after recovery: " + result.duplicateCount);
        } finally {
    ...
    private static ReadResult runIntegrationTest(boolean failover, String failureMarkerPath) throws Exception {
        ...
        String project = "flink-sink-it-" + suffix;
        String logstore = "sink-logstore";
        String topic = failover ? "flip27-sink-failover" : "flip27-sink";
        ...
        try {
            ...
            List<String> expected = expectedIds(recordCount());
            runSink(endpoint, accessKeyId, accessKeySecret, project, logstore, topic,
                    expected, failover, failureMarkerPath);
            ReadResult result = runSource(endpoint, accessKeyId, accessKeySecret, project, logstore, expected.size());

            assertEquals(new HashSet<>(expected), result.records);
            if (!failover) {
                assertEquals("Sink/source roundtrip emitted duplicate ids", 0, result.duplicateCount);
            }
            return result;
        } finally {
    ...
    private static void runSink(
        ...
        AliyunLogSink<String> sink = AliyunLogSink.<String>builder()
                ...
                .setSerializer(new IdSinkSerializationSchema(topic))
        ...
        DataStream<String> stream = env.fromCollection(records);
        if (failover) {
            stream = stream.map(new FailingOnceMap(FAILOVER_AFTER_RECORDS, failureMarkerPath));
        }
        stream.sinkTo(sink).name("aliyun-log-sink-it");
        env.execute("aliyun-log-sink-it");
        System.out.println("Sink wrote " + records.size() + " records through AliyunLogSink");
    }

    private static ReadResult runSource(
            String endpoint,
            String accessKeyId,
            String accessKeySecret,
            String project,
        ...
        try (CloseableIterator<String> iterator = env.fromSource(
                        source,
                        WatermarkStrategy.noWatermarks(),
                        "aliyun-log-sink-verify")
                .executeAndCollect()) {
        ...
        System.out.println("Source verified " + actual.size() + " unique sink ids, duplicates=" + duplicateCount);
        return new ReadResult(actual, duplicateCount);
    }

    private static List<String> expectedIds(int recordCount) {
        List<String> expected = new ArrayList<>(recordCount);
        for (int i = 0; i < recordCount; i++) {
            expected.add(String.format("sink-msg-%05d", i));
        }
        return expected;
    }
    ...
    private static void createProject(Client client, String project) throws LogException {
        client.CreateProject(project, "aliyun-log-flink-connector sink integration test");
    ...
    private static class IdSinkSerializationSchema implements AliyunLogSerializationSchema<String> {
        private final String topic;

        private IdSinkSerializationSchema(String topic) {
            this.topic = topic;
        }
        ...
        public void serialize(String element, Collector<SinkRecord> output) {
            ...
            item.PushBack("id", element);
            item.PushBack("message", "sink integration " + element);
            SinkRecord record = new SinkRecord();
            record.setTopic(topic);
            record.setSource("codex-sink-integration");
    ...
    private static class FailingOnceMap extends RichMapFunction<String, String> {
        ...
        public String map(String value) throws Exception {
            ...
            if (seen >= failAfterRecords && !Files.exists(Paths.get(failureMarkerPath))) {
                ...
                throw new RuntimeException("Intentional failure for sink failover test");
            }
            return value;
        }
    }
